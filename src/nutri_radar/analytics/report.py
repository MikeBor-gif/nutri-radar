...
def format_comparison(scores: list[Score], title: str, note: str) -> str:
    ...
    # Отрицательный обгон называется словами, а не оставляется читателю
    # вычитать знак. Подход хуже строчки «всегда самый частый класс» —
    # это вывод майлстоуна, а не деталь таблицы.
...
def _leakage_note(target: str) -> str:
    """Почему в признаках нет ничего, кроме текста.

    Формулировка зависит от задачи: `nutriscore_grade` считается
    по нутриентам, и подмешать их значило бы вывести формулу заново.
    У `nova_group` риск другой — она про степень переработки, и утечкой
    была бы категория продукта, а не нутриенты. Одна фраза на обе задачи
    врала бы в половине случаев.
    """
    ...
    return (
        "Признаки собраны **только из текста состава**. Ни категории продукта, "
        "ни нутриентов: `nova_group` описывает степень переработки, и категория "
        "предсказывала бы её напрямую, минуя состав."
    )
