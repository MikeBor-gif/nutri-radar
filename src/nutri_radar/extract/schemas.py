...
class SkipReason(StrEnum):
    """Почему состав не превратился в разбор.

    Все четыре случая дают строку с `unreadable=true`, и без этой пометки
    они в таблице неразличимы. А лечатся по-разному: `TOO_LONG` — размером
    контекста, `OUTPUT_LIMIT` — лимитом вывода, `MODEL_UNREADABLE` — промптом
    или моделью. Доля пропусков входит в обязательные метрики майлстоуна,
    и без разбивки по причинам это число нечего обсуждать.

    В БД уезжает строкой, а не типом Postgres: список причин будет пополняться
    по мере встреченных случаев, и миграция ради каждого нового значения —
    цена без выгоды (тот же довод, что и у `kind` в словаре алиасов).
    """
