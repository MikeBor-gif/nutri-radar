...
class SampleItem(BaseModel):
    """Продукт, попавший в выборку для разметки.

    Коммитится в репозиторий: состав выборки — часть эксперимента. Если он
    пересоздаётся при каждом запуске, метрики разных прогонов считаются
    ...
    # Сколько ингредиентов не осилил парсер OFF. По этому полю метрики режутся
    # на «где таксономия справилась» и «где нет» — ровно то разделение, ради
    # которого ADR-006 смещал корпус.
    unknown_ingredients_n: int = 0
