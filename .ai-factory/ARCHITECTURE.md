...
├── wording.py                # (+) обязательная атрибуция ODbL и запрещённые ярлыки
├── openfoodfacts.py          # (+) ЕДИНСТВЕННЫЙ модуль, ходящий в живой API OFF
├── serve.py                  #     команды запуска точек входа: api | bot | mcp
│
...
│   ├── runtime.py            #     очередь к моделям: одна в VRAM за раз
│   ├── factory.py            #     выбор провайдера по LLM__PROVIDER, возвращает порты
│   └── adapters/
...
│   ├── pipeline.py           #     ЕДИНСТВЕННАЯ сборка конвейера для всех точек входа
│   ├── product_card.py       #     карточка по штрихкоду: корпус, фолбэк в живой API
│   └── cli.py
│
├── agent/                    # ── СЛАЙС: агент ──
│   ├── tools/                #     sql_query, vector_search, lookup_barcode
│   ├── registry.py           #     сборка реестра: один источник описаний инструментов
│   ├── loop.py               #     план → инструмент → проверка, лимиты шагов и стоимости
│   └── cli.py
│
├── api/                      # ── ТОЧКА ВХОДА: FastAPI ──
│   ├── app.py                #     сборка, lifespan, request_id, обработчики ошибок
│   ├── errors.py             #     доменные ошибки в коды HTTP: чей отказ, тот и код
│   ├── schemas.py            #     контракт API отдельно от внутренних структур
│   ├── factory.py            #     готовый объект приложения для uvicorn --reload
│   └── routes/               #     products, search, ask, agent
│
├── bot/                      # ── ТОЧКА ВХОДА: aiogram 3 ──
│   ├── app.py                #     диспетчер, long polling, общие зависимости
│   ├── texts.py              #     всё, что бот говорит от себя; /start несёт ODbL
│   ├── middlewares.py        #     приватность: хэш чата и длина, но не текст
│   ├── keyboards.py          #     инлайн-кнопки; состояние живёт в callback_data
│   ├── barcode_image.py      #     чтение штрихкода с фото, zxing-cpp
│   ├── errors.py             #     доменные ошибки в сообщения пользователю
│   └── handlers/             #     commands, barcode, photo, ask
│
└── mcp_server/               # ── ТОЧКА ВХОДА: MCP ──
    └── server.py             #     три инструмента агента наружу по stdio
...
3. **`openfoodfacts.py`** *(M7)* — живой API OFF это внешняя система, и место
   ей рядом с остальными внешними системами. До M7 вызов жил внутри инструмента
   агента и был единственным. Когда у него появился второй потребитель —
   карточка продукта вне корпуса, — выбор был между копией вызова в слайсе
   `retrieval` и переездом в общий слой. Копия превратила бы одну границу в две;
   зависимость `retrieval` → `agent` запрещена правилами ниже. ADR-031.
4. **`wording.py`** *(M7)* — обязательная атрибуция ODbL, дисклеймер и список
   запрещённых оценочных ярлыков. Требования брифа, сформулированные словами,
   теряются молча; собранные в модуль, они проверяются тестом во всех трёх
   точках входа сразу.
5. **`llm/runtime.py`** *(M7)* — очередь к локальным моделям. На 6 ГБ VRAM две
   модели не помещаются, и в сервисе с параллельными запросами порядок их
   загрузки нельзя держать в голове: он держится кодом или не держится вовсе.
...
- ✅ любой слайс → `openfoodfacts` и `wording` — это общее, как `db` и `llm`
...
- ❌ точка входа импортирует другую точку входа. Общее для них выносится вниз:
  так `build_registry` переехал из `agent/cli.py` в `agent/registry.py`, когда
  реестр понадобился HTTP-API и MCP-серверу
- ❌ обращение к моделям Ollama мимо `llm/runtime.py`. Прямой вызов адаптера
  работает и в одиночку выглядит правильным — ломается он только под
  параллельной нагрузкой, то есть в бою, а не в тесте
- ❌ сборка конкретного адаптера в модуле логики слайса. Реализацию выбирает
  `llm/factory.py` по `LLM__PROVIDER`; собранный руками `OllamaLLM` молча
  игнорирует настройку — код работает, просто не на той модели, что в `.env`.
  Проверяется тестом `test_llm_factory.py`, а не вычиткой
