...
- [x] **Задача 7:** Команда `nutri-radar health` — пять проверок: соединение с
...
- [x] **Задача 8:** Тесты — `conftest.py` (настройки из словаря, тестовая БД с
...
- [x] **Задача 11:** Приёмка DoD — прогон с чистого тома (`down -v` → `up` →
