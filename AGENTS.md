...
│   │   ├── language.py       ✓   язык текста по алфавиту; знает свои границы
│   │   ├── metrics.py        ✓   свойство выдачи, подтверждённость, язык, отказ вне домена
│   │   ├── switching.py      ✓   замер цены смены модели на 6 ГБ VRAM
...
| `src/nutri_radar/retrieval/language.py` | *(M7)* признак языка по алфавиту. **Различает только `ru` и `en`**: немецкий и французский для него неотличимы от английского, и вызывающий код обязан это учитывать — иначе метрика запишет в промахи собственное ограничение |
| `src/nutri_radar/retrieval/prompts/rag_v2.md` | *(M7)* провалившийся эксперимент, оставленный намеренно: принуждение языка схемой сработало, но модель ушла в вырожденный повтор. Версия по умолчанию — `rag_v1`, ADR-032 |
