...
>
> **Текущее состояние: M0 и M1 выполнены, ждут ревью.** В базе аналитический
> корпус на 147 842 продукта. Работают конфигурация, слой БД, миграции,
> health-check, слайс `ingest` и 158 тестов. Слайсы `extract`, `evals`,
> `analytics`, `retrieval`, `agent` и точки входа `api`, `bot`, `mcp_server`
> появляются со своими майлстоунами.
